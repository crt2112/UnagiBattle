...
public class Enemy_A_MeganeManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        _rbody = GetComponent<Rigidbody2D>();
            ...
            targetObject = PlayerManager.GetGameObject();
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        ...
        // ヒット判定しない対象物か確認。自分自身にヒットしないようにチェック。
        if ((other.isTrigger ||
             (other.tag == "Wall") ||
             (other.tag == "Floor") ||
             ...
             (ownwer.tag == "Enemy" && other.tag == "Enemy_Body") ||
             (ownwer.tag == "Enemy" && other.tag == "EnemyArm") ||
             (ownwer.tag == "Enemy" && other.tag == "EnemyAttack_Arm") ||
             (ownwer.tag == "Enemy" && other.tag == "Enemy_Muzzle1") )
           )
        {
            return;
        }
    ...
    void FixedUpdate()
    {
        ...
        switch (fireType)
        {
            case FIREBULLET.ANGLE: // 指定した角度に発射
                _rbody.velocity = Quaternion.Euler(0.0f, 0.0f, angle) * new Vector3(speed, 0.0f, 0.0f);
                break;
            ...
            case FIREBULLET.HOMING: // 完璧にホーミング
                {
                    ...
                    _rbody.velocity = Quaternion.Euler(0.0f, 0.0f, angle) * vecMove;
                }
                break;
            ...
            case FIREBULLET.HOMING_Z: // 指定した角度内でホーミング
                ...
                _rbody.velocity = (homingRotate * Vector3.right) * speed;
                break;
        }
